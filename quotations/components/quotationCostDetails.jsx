...
const QuotationCostDetails = ({
  ...
  useEffect(() => {
    //update accommodations costs .................
    if (values.itinerary.length == 0 && itinerary.length > 0) {
      //itinerary was empty b4.....not anymore...
      const accmCosts = itinerary.map((itm) => ({
        code: itm.code,
        cost: 0,
      }));
      setAllAccommodationsCosts(accmCosts);
    } else if (itinerary.length === 0) {
      //itinerary is empty now...........
      setAllAccommodationsCosts([]);
    } else if (itinerary.length > values.itinerary.length) {
      //new itinerary item (day) added....................
      setAllAccommodationsCosts((prevState) => [
        ...prevState,
        {
          code: itinerary[itinerary.length - 1].code,
          cost: 0,
        },
      ]);
    } else if (itinerary.length < values.itinerary.length) {
      //one itinerary item has been removed.................
      setAllAccommodationsCosts((prevState) => {
        const newList = prevState.filter((itm) =>
          itinerary.some((itnItem) => itnItem.code === itm.code)
        );
        return newList;
      });
    }
  ...
  const [allAccommodationsCosts, setAllAccommodationsCosts] = useState([]);

  const [isCostDetailsVisible, setIsCostDetailsVisible] = useState(false);

  //itinerary cost update............................................................

  const handleUpdateAccommodationCosts = (costDetails) => {
    setAllAccommodationsCosts((prevState) => {
      const updatedList = prevState.map((itm) => {
        if (itm.code === costDetails.code) {
          return costDetails;
        }
        return itm;
      });
  ...
  const getAccmRate = (item) => {
    ...
    console.log("the accm item", item.accommodation);
    const selectedRoom =
  ...
  const getTotalAccmCost = (code) => {
    const itm = allAccommodationsCosts.find((itm) => itm.code === code);

    return itm ? itm.cost : 0;
  };

  const getTotalAccommodationsCost = (code, accommodations, itnIndex) => {
    if (
      accommodations.some((accm) => {
        return accm.category === localSettings.campSite;
      })
    ) {
      //..campsite..............
      const items = itinerary[itnIndex].campingCharges
        .map((itm) => itm.amount)
        .filter((itm) => itm);

      return arrayTotal(items);
    } else {
      //..no campsite...........
      return getTotalAccmCost(code);
    }
  };

  const getDayTripsCosts = (tripIds) => {
    const costs = tripIds.reduce((accumulator, currentItem) => {
      const trip = findDayTrip(currentItem);
      const currentAmount = trip ? trip.price.foreigner : 0;
      return (
        accumulator +
        (Number(kidsNumber) + Number(adultsNumber)) * Number(currentAmount)
      );
    }, 0);
          ...
          <div className={`${isCostDetailsVisible ? "block" : "hidden"}`}>
            ...
            <div className="mt-2">
              {values.itinerary.map((item, index) => {
                ...
                const accmTotal = getTotalAccommodationsCost(
                  item.code,
                  item.accommodation,
                  index
                );
                // const accmTotal =
                //   item.accommodation.length > 0 ? getAccmRate(item) : 0;
                ...
                //day trips costs.....   .......... ..........  ...........
                const dayTripsCosts = getDayTripsCosts(item.dayTrips);

                const generalAmount =
                  otherCostsTotal + accmRate + dstnTotal + dayTripsCosts;
                ...
                return (
                  <div key={index}>
                    ...
                    <div className="grid gap-6 md:grid-cols-7 mt-3">
                      ...
                      {item.accommodation.length > 0 && (
                        <div className="md:col-span-3">
                          <AccommodationCosts
                            isForeigner={isForeigner}
                            isHighSeason={isHighSeason}
                            code={item.code}
                            accommodations={item.accommodation}
                            onHandleUpdateAccmCosts={
                              handleUpdateAccommodationCosts
                            }
                            charges={item.campingCharges}
                            itnItemIndex={index}
                    ...
                    {item.dayTrips.length > 0 && (
                      <div className="mt-5">
                        <h2 className="block mb-2 text-lg text-gray-700 font-bold">
                          Paid Activities
                        </h2>

                        {item.dayTrips.map((tripId) => {
                          const trip = findDayTrip(tripId);
                          return (
                            <div key={tripId} className="flex gap-2">
                              <p>{trip ? trip.title : ""}</p>
                              <p>
                                (
                                {`${trip.price.foreigner} * (${
                                  Number(kidsNumber) + Number(adultsNumber)
                                } people)`}
                                ) = $
                                {trip.price.foreigner *
                        ...
                        <p className="font-semibold">
                          {" "}
                          Total: {changeToCurrency(dayTripsCosts)}
                        </p>
                      </div>
                    )}
            ...
            {values.transport.length > 0 && (
              <div className="mt-14">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  {values.transport
                    .filter((itm) => (itm.carId ? true : false))
                    .map((itm, carIndex) => {
